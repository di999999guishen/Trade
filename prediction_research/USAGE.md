...
2. 更新新闻事件；
3. 运行资金流粗筛，按同指数／同赛道每组最多1只去重，冻结全部合格候选（默认无数量上限）；
...
`cycle --top` 控制进入量化预测的候选数量，默认读取配置为20；粗筛默认无数量上限。`screen-etfs --limit 20` 可单独限制粗筛。LLM预算仍为5，cycle内当前按配置跳过。项目根目录也可使用 `python run_research.py cycle` 加载本机环境配置并启用国内域名代理旁路。
