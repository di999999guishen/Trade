...
2026-09-15 C盘合并：粗筛按同指数／同赛道去重，默认无总数量上限；量化申请仍为前20，大模型预算仍为5。五档资金流、国内数据源与六因子／冻结证据传递已合并。验收及冲突取舍见 [合并记录](docs/C_DRIVE_MERGE_20260915.md)。

2026-09-15 更新：默认量化覆盖前20候选，新增六类观察/机会与风险榜、按日期隔离校准、概率回退诊断及每轮文件归档。完整实现、测试、回测结果和后续步骤见 [整改实施手册](docs/ETF_REMEDIATION_20260915.md)，每日备份见 [归档与恢复指南](docs/ETF_ARCHIVE_GUIDE.md)。
...
当前包含量价对照模型与商品外部特征模型。是否通过质量门按本轮、候选池和周期分别判断；基率回退、未校准和验证失败分别显示。9月15日固定五只历史候选对照中，新5日模型仍未超过历史上涨率基线，新20日模型通过条件数值门槛；这不等于动态粗筛策略或前瞻交易已有效。历史回测不能替代上线后的前瞻记录。
