...
def test_fetch_returns_normalizes_symbol(monkeypatch):
    ...
    # Exercise the real domestic/Yahoo history router without constructing LLMs.
    raw, alpha, days, resolved = TradingAgentsGraph._fetch_returns(
        object.__new__(TradingAgentsGraph), "XAUUSD", "2025-01-02", holding_days=5, benchmark="SPY"
    )
