...
from tradingagents.dataflows.eastmoney_data import resolve_cn_instrument_identity
...
def resolve_instrument_identity(ticker: str) -> dict:
    ...
    # A股/港股标的优先走国内源解析名称（akshare/东方财富），避免 yfinance
    # 对 A股 ETF 的限流与错误身份识别。
    try:
        cn_identity = resolve_cn_instrument_identity(ticker)
        if cn_identity:
            return cn_identity
    except Exception as exc:  # noqa: BLE001 — fail open, fall through to yfinance
        logger.debug("Could not resolve CN identity for %s: %s", ticker, exc)
